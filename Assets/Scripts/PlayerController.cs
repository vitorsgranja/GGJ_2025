...
public class CharacterController2D : MonoBehaviour
{
	...
	private Color currentSecondaryWeaponColor;
	...
	private void Start()
	{
		...
		currentSecondaryWeaponColor = secondaryWeaponColor;
	...
	private IEnumerator ResetInvulnerabilityState()
	{
		...
		int blinkCount = 3;
		float count = 0;
		while (count < blinkCount * 2)
		{
			yield return new WaitForSeconds(waitFor / (blinkCount * 2));
			if (count % 2 == 0)
			{
				currentBaseColor = new Color(1f, 0.5f, 0.5f, 0.5f);
				currentSecondaryWeaponColor = currentBaseColor;
			}
			else
			{
				currentSecondaryWeaponColor = secondaryWeaponColor;
				currentBaseColor = currentWeaponIndex == 0 ? baseColor : secondaryWeaponColor;
			}
			count++;
		}
		currentBaseColor = baseColor;
	...
	private void UpdatePlayerColor()
	{
		if (currentWeaponIndex == 0)
		{
			...
			TailColor(currentBaseColor);
		}
		else
		{
			float intensity = Mathf.Clamp01(secondaryWeaponAmmo / 10f);
			spriteRenderer.color = Color.Lerp(currentBaseColor, currentSecondaryWeaponColor, intensity);
			TailColor(Color.Lerp(currentBaseColor, currentSecondaryWeaponColor, intensity));
		}
	...
	private void ApplyKnockback(Vector2 knockback)
	{
		...
		StartCoroutine(nameof(ResetKnockback));
	}
